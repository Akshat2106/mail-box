...
const Inbox = () => {
  ...
  const showEmail = (mail) => {
    ...
    sendRequest({
      url: `https://mail-box-ebf7e-default-rtdb.firebaseio.com/emails/${mail.id}.json`,
      method: "PUT",
      body: {
        ...mail,
        read: true,
  ...
  const deleteMail = (id) => {
    sendRequest({
      url: `https://mail-box-ebf7e-default-rtdb.firebaseio.com/emails/${id}.json`,
      method: "DELETE",
    });
  };
        ...
        <Container className="mb-5">
          ...
          <ListGroup>
            {emails.filter((mail) => mail.receiver === email).length === 0 && (
              <ListGroup.Item className="text-center border-dark">
            ...
            {emails
              ...
              .map((mail) => (
                <div key={mail.id} className="d-flex mb-1">
                  <ListGroup.Item
                    className="d-flex align-items-center border-dark rounded"
                    action
